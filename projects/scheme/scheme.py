...
def make_call_frame(procedure, args, env):
    ...
    # BEGIN Question 12
    if type(procedure) is MuProcedure: 
...
def check_formals(formals):
    ...
    list = []
    while formals:
        if formals.first in list or not scheme_symbolp(formals.first):
            raise SchemeError("list of formals is not a well-formed list of symbols or if any symbol is repeated")
