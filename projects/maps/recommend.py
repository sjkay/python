...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        centroids = [find_centroid(restaurant) for restaurant in clusters if len(restaurant)!=0]
        # END Question 6
        n += 1
    return centroids
...
def find_predictor(user, restaurants, feature_fn):
    ...
    meanx = mean(xs)
    meany = mean(ys)
    S_xx = sum([pow(x - meanx, 2) for x in xs])
    S_yy = sum([pow(y - meany, 2) for y in ys])
    S_xy = sum([(x-meanx)*(y-meany) for x,y in zip(xs,ys)])
    b, a, r_squared = S_xy/S_xx, meany - S_xy/S_xx * meanx, pow(S_xy, 2)/(S_xx*S_yy)  # REPLACE THIS LINE WITH YOUR SOLUTION
    # END Question 7
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    return max([find_predictor(user, reviewed, fn) for fn in feature_fns], key = lambda find_predictor: find_predictor[1])[0]
    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    for restaurant in restaurants:
        rname = restaurant_name(restaurant)
        if restaurant in reviewed:
            dict[rname] = user_rating(user, rname)
        else:
            dict[rname] = predictor(restaurant)
    return dict
    # END Question 9
