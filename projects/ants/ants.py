...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 7
            if type(insect) is BodyguardAnt and insect.ant:
                self.ant = insect.ant
            else:
                self.ant = None
            # END Problem 7
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        i = 0
        p = self.place
        while p is not hive or p.entrance is not None:
            if len(p.bees)> 0 and i >= self.min_range and i <= self.max_range:
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        if self.i > 1:
            ...
            colony.queen = QueenPlace(colony.queen, self.ant_queen)
            ant_ant_queen = self.ant_queen.exit
            while ant_ant_queen:
                if ant_ant_queen.ant:
                    if type(ant_ant_queen.ant) == BodyguardAnt:
                        if ant_ant_queen.ant.ant:
                            ant_ant_queen.ant.damage = type(ant_ant_queen.ant.ant).damage * 2
                    else:
                        ant_ant_queen.ant.damage = type(ant_ant_queen.ant).damage * 2
                ant_ant_queen = ant_ant_queen.exit
        # END Problem 9
